...
class Model {
	...
	public function getAllActiveComplaints(){
		...
		$stmnt = $this->db->prepare("
			SELECT t1.* , t3.emergency_level,t3.alloted_time,t3.type as 'natureType'
				...
				AND t1.completed = 1
				ORDER BY t1.type,t1.dateadded DESC,FIELD(t3.emergency_level,'High','Medium','Low'),t1.brgy ASC
			");
	...
	public function superScheduler($lineman){
		...
		foreach($lineman as $idx => $l){
			$start_date = new DateTime($l['start']);
			$oldDate  = new DateTime($l['end']);
			$since_start = $start_date->diff($oldDate);
			$found = false;

			do {
				$check = $start_date->format("D M d Y H:i:s");
				$exists = $this->db->query("
						SELECT *
						FROM schedule
						WHERE startdate = '".$check."'
					")->rowcount();
			...
			$remaining = $minutes;
			$lastHour = date("H",strtotime($l['start']));
			$lastMin = date("i",strtotime($l['start']));
			
			$startDate = $start_date;
			$finalDay = date("D M d Y");

			$d = date("D", strtotime($startDate->format("D M d Y H:i:s")));
			// $d = date("Y-m-d H:i:s", strtotime($startDate->format("D M d Y H:i:s")));
			//check first if date exists
			$day = 0;
		    switch($d){
			...
			$week_number = date("W", strtotime($startDate->format("D M d Y H:i:s")));
			$year = date("Y", strtotime($l['start']));
			$equivalentDate = strtotime($year."W".str_pad($week_number,2,0,STR_PAD_LEFT).$day);
			$complaint = $this->getAllActiveComplaints();

			if($lastLineman == $l['userid']){

				//if lastlineman after break is the as previous lineman, get the diff between last end date and new date, the get equiv date
				//if lineman!=inspector, reset date
				//TODO:new date here
				$nextDay = date("l", strtotime($l['start']));
				$finalDay = date('D M d Y', strtotime("next $nextDay", strtotime($previousDay)));
			}

			$finalDay2 = date('l', $equivalentDate);

			$linemanStartDay = date("l", strtotime($l['start']));
			$startDateFound = false;

			$startCounter = 0;
			while($startDateFound === false) {
				if($finalDay2 == $linemanStartDay){
					$startDateFound = true;
					foreach($complaint as $idx2 => $c){
						// if next day != lineman start date, break 1,startdate + 1 day
						// $currentDay = $startDate->format("l");
						
						if(($complaint[$c['idx']]['added']) === false){
							//check first if in stock
							$inStock = $this->isInStock($c['brgy'], $c['complaint_nature']);

							if($inStock === true){
								if(trim(strtolower($c['municipality'])) == trim(strtolower($l['municipality']))){
									// if(!isset($c['done'])){				
										if( (($l['userRole'] == "line_man") && ($c['natureType'] == "complaint")) || 
											(($l['userRole'] == "inspector") && ($c['natureType'] == "request")) 
												...
												if($c['alloted_time'] <= $remaining){
													if($startCounter == 0){
														$initTime = strtotime(date("H:i"));
														$startInitTime = strtotime($startDate->format("H:i"));

														$xx = date("m-d-Y H:i:s", $initTime);
														$yy = date("m-d-Y H:i:s", $startInitTime);

														if($initTime > $startInitTime){
															$startDate = date_create($startDate->format("D M d Y") . date("H:i:s", $initTime));
														}

													}
													
													$remaining -= $c['alloted_time'];
													$start = date_create((date("Y-m-d", $equivalentDate)." ".$lastHour.":".$lastMin.":00"));

													$h = (floor($c['alloted_time']/60) > 0) ? floor($c['alloted_time']/60) : "00" ;
													$m = $c['alloted_time'] %60;
													$finalStart = $finalDay." ".$startDate->format("H:i:s");

													$newDate = date_add(date_add($start,date_interval_create_from_date_string("$h hours")),date_interval_create_from_date_string("$m minutes"));
													
													$finalEnd = $finalDay." ".$newDate->format("H:i:s");

													//if finalStart <$linemanEndtime and finalEnd < $linemanEnd, else break 1
													// if($finalDay2 == $linemanStartDay){
														$complaint[$c['idx']]['filter_level'] = 4;
														// $complaint[$c['idx']]['added'] = true;
														$records[] = array(
																// "complaintId" => $c['id'],
																// "userId" => $l['id'],
																"start" => $startDate,
																"end" => $newDate,
																"nature" => $c['complaint_nature'],
																"alotted" => $c['alloted_time']
															);
														
														// if start date is > end date due to the adjusted time
														echo "__________________________<pre>";
														var_dump($finalStart);	
														echo "<pre>";
														var_dump($finalEnd);	
														echo "___________________________";

														if(strtotime($finalStart) >= strtotime($finalEnd)){
														echo "===========================<pre>";
															break;
														}
														...
														// $stmnt->execute(array($l['userid'], $c['id']));

														$previousDay = $newDate->format("H:i:s");
														$startDate = $newDate;
														$lastHour = $newDate->format("H");
														$lastMin = $newDate->format("i");
														$startCounter++;
														// $this->getLinemanMessageForAutoScheduling($startDate->format("D M d Y H:i:s"), $c['id'], $l['userid']);
													// } else {
													// 	echo "=============d2<pre>";
													// 	print_r($finalDay2);
													// 	$newDate2 = date_add($newDate,date_interval_create_from_date_string("1 day"));
													// 	print_r($linemanStartDay);

														
													// 	$previousDay = $newDate2->format("H:i:s");
													// 	$startDate = $newDate2;
													// 	$lastHour = $newDate2->format("H");
													// 	$lastMin = $newDate2->format("i");
													// 	// break;
													// }
												} else {
									...
									// } else {
										// echo "d pumasok ung top";
									// }

								} else {
									if($complaint[$c['idx']]['filter_level'] <= 2){
										$complaint[$c['idx']]['filter_level'] = 2;
									...
									$stmnt->execute(array(
											"No Branch assigned to this address yet.",
											$c['id']
										));
									}
									
								}
								//reason : no branch assigned for consumer's brgy address
							} else {
								...
								if($complaint[$c['idx']]['filter_level'] <= 1){
									...
									$stmnt->execute(array(
											"Insufficient Supply for this nature of complaints",
											$c['id']
										));
								}
					...
					//+1 to startdate
					// echo "+1==============================<pre>";
					$equivalentDate = strtotime("+1 day", $equivalentDate);
					// echo "<pre>";
					// $t = date("d-m-Y", $equivalentDate);
					$finalDay2 = date("l", $equivalentDate);
					// var_dump($finalDay2);
				}
			}
	...
	public function addSmsToComplaint($sms){
		$record = (array) $sms;
	...
	public function processComplaint($data){
		...
		if(count($this->errors) > 0){
			...
			$id = $this->addSmsToComplaint($data);
